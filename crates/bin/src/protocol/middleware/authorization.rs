...
use crate::protocol::errors::ApiError;
...
/// Authorization middleware with a shared password.
///
/// Mirrors `createRoutes(password)` in `handlers/serve.ts` — the password is
/// provided as axum [`State`] and compared against the request's
/// `Authorization: Bearer <password>` header. PTY connect requests with a
/// ticket query parameter bypass the check.
pub async fn authorization_with_password(
    ...
    if !authorized {
        return Err(ApiError::unauthorized("Invalid or missing credentials"));
    }
