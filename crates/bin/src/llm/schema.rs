...
///
/// Top-level keys are provider names (`"openai"`, `"anthropic"`, …) mapping
/// to opaque per-provider JSON objects. This mirrors the TS
/// `ProviderMetadata = Record<string, Record<string, unknown>>`.
pub type ProviderMetadata = serde_json::Map<String, serde_json::Value>;

pub fn provider_metadata(provider: &str, value: serde_json::Value) -> Option<ProviderMetadata> {
    let mut map = serde_json::Map::new();
    map.insert(provider.to_string(), value);
    Some(map)
}

pub fn provider_metadata_get<'a>(
    metadata: &'a Option<ProviderMetadata>,
    provider: &str,
) -> Option<&'a serde_json::Value> {
    metadata.as_ref()?.get(provider)
}
...
impl ToolOutput {
    pub fn new(structured: serde_json::Value, content: Vec<ToolContent>) -> Self {
        Self { structured, content }
    }

    pub fn from_result_value(result: &ToolResultValue) -> Option<Self> {
        match result {
            ToolResultValue::Json { value } => Some(Self::new(value.clone(), Vec::new())),
            ...
            ToolResultValue::Content { value } => Some(Self::new(
                serde_json::Value::Object(serde_json::Map::new()),
                value.clone(),
            )),
...
pub fn tool_result_text(value: &serde_json::Value) -> String {
...
pub fn tool_result_part_text(result: &ToolResultValue) -> String {
    match result {
        ToolResultValue::Text { value } => tool_result_text(value),
        ToolResultValue::Error { value } => tool_result_text(value),
        ToolResultValue::Json { value } => serde_json::to_string(value).unwrap_or_default(),
        ToolResultValue::Content { value } => value
            .iter()
