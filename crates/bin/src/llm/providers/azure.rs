...
impl AzureProvider {
    pub fn from_env() -> Option<Self> {
        ...
        Some(Self { api_key, endpoint, deployment, client: reqwest::Client::new() })
    }
}
...
fn convert_request(req: &LlmRequest) -> ChatCompletionRequest {
    let messages: Vec<ChatCompletionMessage> = req.messages.iter().map(|m| {
        let content: String = m.content.iter().filter_map(|p| match p {
            ContentPart::Text(t) => Some(t.text.clone()), _ => None,
        }).collect();
        ChatCompletionMessage {
            role: match m.role { MessageRole::System => ChatCompletionRole::System, MessageRole::Assistant => ChatCompletionRole::Assistant, MessageRole::Tool => ChatCompletionRole::Tool, _ => ChatCompletionRole::User },
    ...
    ChatCompletionRequest {
        model: req.model.id.clone(), messages, temperature: gen.and_then(|g| g.temperature),
        top_p: gen.and_then(|g| g.top_p), n: None, stream: Some(false),
        stop: gen.and_then(|g| g.stop.clone()).map(|s| if s.len()==1 { StopSequence::Single(s[0].clone()) } else { StopSequence::Multiple(s) }),
        max_tokens: gen.and_then(|g| g.max_tokens), max_completion_tokens: None,
        presence_penalty: None, frequency_penalty: None, logit_bias: None, user: None,
        tools: None, tool_choice: None, seed: gen.and_then(|g| g.seed), response_format: None,
    }
}
...
impl LlmProvider for AzureProvider {
    ...
    async fn generate(&self, request: &LlmRequest) -> Result<LlmResponse, LlmError> {
        ...
        let url = format!("{}/openai/deployments/{}/chat/completions?api-version=2024-02-15-preview", self.endpoint.trim_end_matches('/'), self.deployment);
        let resp = self.client.post(&url).bearer_auth(&self.api_key).json(&body).send().await.map_err(|e| LlmError::network(e.to_string()))?;
        if !resp.status().is_success() { let s = resp.status(); let t = resp.text().await.unwrap_or_default(); return Err(LlmError::provider(format!("Azure error {}: {}", s, t))); }
    ...
    async fn stream(&self, request: &LlmRequest) -> Result<LlmResponse, LlmError> {
        ...
        let resp = self.client.post(&url).bearer_auth(&self.api_key).json(&body).send().await.map_err(|e| LlmError::network(e.to_string()))?;
        if !resp.status().is_success() { let s = resp.status(); let t = resp.text().await.unwrap_or_default(); return Err(LlmError::provider(format!("Azure error {}: {}", s, t))); }
        let mut full = String::new(); let mut fr = None; let mut usage = None;
            ...
            while let Some(pos) = buf.find('\n') {
                ...
                if let Some(data) = line.strip_prefix("data: ") {
                    ...
                    if let Ok(chunk) = serde_json::from_str::<ChatCompletionChunk>(data) {
                        for c in chunk.choices { if let Some(content) = c.delta.content { full.push_str(&content); } if let Some(r) = c.finish_reason { fr = Some(r); } }
                        if let Some(u) = chunk.usage { usage = Some(Usage { input_tokens: Some(u.prompt_tokens), output_tokens: Some(u.completion_tokens), total_tokens: Some(u.total_tokens), ..Default::default() }); }
                    }
                }
            }
        }
